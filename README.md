...
- **`/analyse` — Issue, PR, Discussion and repo health**
  ...
  ```bash
  # Analyse any thread by number — auto-detects issue/PR/discussion, wide-net related search
  /analyse 123
  # Repo health overview with duplicate clustering across issues+PRs+discussions
  /analyse health
  # Downstream consumer impact analysis
  /analyse ecosystem
  # Analyse + draft contributor reply
  /analyse 123 --reply
  ```
...
- **`/develop` — Unified development orchestrator**
  ...
  - `plan` — scope analysis only; produces a structured plan in `tasks/plan_<slug>.md`, then has execution agents review it for autonomous executability; terminal output includes file link, summary, and steps table
