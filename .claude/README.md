...
<details>
<summary><strong>`/optimize` — Performance deep-dive</strong></summary>
...
<details>
<summary><strong>`/research` — Autonomous metric improvement loop</strong></summary>

```bash
/research plan "increase test coverage to 90%"    # interactive wizard
/research "improve test coverage"                  # run iteration loop
/research resume                                   # resume most recent run
/research "reduce training time by 20%" --team     # team mode: parallel optimization axes
/research "improve validation accuracy" --colab    # GPU workloads via Colab MCP (opt-in)
```
...
| `/research --team`        | Goal spans multiple optimization axes (speed = arch + pipeline + compute) |
