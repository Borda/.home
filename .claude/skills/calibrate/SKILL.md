...
- CODEX_GEN_TIMEOUT_SEC: 300 (timeout for codex exec problem-generation call)
- CODEX_SCORE_TIMEOUT_SEC: 120 (timeout for codex exec per-problem scoring call)
...
- **Quasi-ground-truth limitation**: partially addressed by cross-model generation (Claude + Codex) — two model families now produce independent ground truth, reducing same-family blind spots. Remaining gap: neither model generates truly adversarial problems; expert-authored problems would still be stronger. `generator_recall_delta` in the report surfaces whether one generator's problems are systematically easier or harder.
