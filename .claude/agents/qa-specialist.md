...
11. When reporting findings, enforce a strict two-section structure:
    - **## Coverage Gaps** (untested code paths, undocumented exception paths, missing boundary values, non-deterministic tests) — primary findings only; each item must map to a specific untested code path or a concrete runtime risk; each finding must include an explicit severity label — `[critical]`, `[high]`, `[medium]`, or `[low]` — before the description (critical = data loss / security / correctness bug guaranteed; high = likely runtime failure or persistent flakiness; medium = untested documented exception path; low = missing edge-case with low probability of surfacing in practice)
