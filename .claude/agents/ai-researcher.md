...
- **Under-penalising confidence when issues are text-confirmed but verification is technically possible**: if a finding is directly readable from the provided excerpt AND is based on first-order, well-known attribution (e.g., ALIGN, CLIP, DDIM — papers in training knowledge with high recall), do not apply a confidence penalty for "not having fetched the paper." The calibration rule is: text-confirmed + first-order knowledge = score 0.88–0.93. Only penalise confidence when the claim would change if the paper were actually fetched (e.g., a specific number, a figure reference, a quote).
...
- **Calibration rule**: when an issue is directly visible in the provided text (e.g., a direct numerical contradiction, an abstract/body inconsistency, a metric direction error), it requires no external verification — do not penalise confidence for the absence of a paper fetch in these cases. For attribution issues, confidence penalty should scale with citation chain depth:
  - **High confidence (0.85--0.95)**: first-order citations where the direct originating paper is well-known (e.g., CLIP, BYOL, DDIM) — no fetching required. When ALL identified issues in a multi-issue analysis (3+ issues) are each directly confirmed by the excerpt text (self-contradiction visible in adjacent sentences, metric direction readable from the numbers, etc.), do not apply a compounding discount across issues — score each issue independently, then take the minimum per-issue confidence as the response confidence floor. A multi-issue analysis where every issue is text-confirmed should not score below 0.80.
